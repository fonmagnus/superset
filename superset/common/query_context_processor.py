    ...
    def get_viz_annotation_data(  # noqa: C901
        ...
        try:
                ...
                if not chart.datasource:
                    raise QueryObjectValidationError(
                        _(
                            ...
                            " annotation layer '%(layer_name)s') was"
            ...
            if not (query_context := chart.get_query_context()):
                raise QueryObjectValidationError(
                    _(
                        "The query context for chart ID %(chart_id)s"
                        " (referenced by annotation layer '%(layer_name)s')"
                        " was not found. Please ensure the chart is properly"
